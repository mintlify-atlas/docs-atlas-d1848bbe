---
title: 'Build UI with Odoo OWL Components'
sidebarTitle: 'OWL Components'
description: 'Learn how to build Odoo web UI with OWL: class-based components, lifecycle hooks, QWeb templates, slots, and Odoo-specific hooks like useBus and useService.'
---
...
<CardGroup cols={2}>
  ...
  <Card title="CheckBox" icon="check-square">
    Simple checkbox with a clickable label. Linked via `t-on-change`.
  </Card>
...
Connects a component's pagination state to the control panel's `Pager` component:
...
Positions a *popper* element relative to a *reference* element, updating on scroll and resize:
...
Activates spellcheck on focused `input`, `textarea`, or `contenteditable` elements:
