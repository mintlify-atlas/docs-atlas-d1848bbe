...
| `X-Odoo-Database` | When needed | Database name (required on multi-database servers) |
...
requests.post(
    ...
    json={"key": old_api_key},  # revoke old key after successful rotation
)
```

<Tip>
**Key rotation best practice:** Generate the new key first, verify all services use it, then revoke the old key. Use separate API keys per service to simplify auditing and revocation.
</Tip>
